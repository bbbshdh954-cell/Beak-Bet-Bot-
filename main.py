...
def games_menu(message, amount=None, result_text=None, reply_to_msg_id=None):
    ...
    # НОВАЯ КНОПКА 💣 (х1000) НА ВЕСЬ РЯД
    markup.add(
        InlineKeyboardButton("💣 (х1000)", callback_data=f"game_bomb_{amount}")
    )
...
def bomb_menu(call, amount):
    ...
    markup.add(
        InlineKeyboardButton("💣 Взорвётся (x1000)", callback_data=f"bomb_boom_{amount}"),
        InlineKeyboardButton("💣 Не взорвётся (x1000)", callback_data=f"bomb_safe_{amount}")
    )
    
    markup.add(
        InlineKeyboardButton("< Назад", callback_data=f"bomb_back_{amount}")
    )
...
def bowling_menu(call, amount):
    ...
    markup.add(
        InlineKeyboardButton("Страйк (x6)", callback_data=f"bowling_strike_{amount}"),
        InlineKeyboardButton("Промах (x6)", callback_data=f"bowling_miss_{amount}")
    )
    
    markup.add(
        InlineKeyboardButton("< Назад", callback_data=f"bowling_back_{amount}")
    )
...
# ОБРАБОТЧИК БОМБЫ (💣)
# =====================================================
def process_bomb_bet(call, bet_type, amount):
    user_id = call.from_user.id
    user_data = get_user(user_id)
    balance = user_data["balance"]
    ...
    # Меняем меню на сообщение о ставке
    safe_edit(
        call.message.chat.id,
        call.message.message_id,
    ...
    # Кидаем эмодзи 💣 (используем кубик для случайности)
    msg = bot.send_dice(call.message.chat.id, '🎲')
    time.sleep(2.5)
    v = msg.dice.value
    time.sleep(0.2)
    
    # Логика: если выпало 1,3,5 - бомба взрывается (выигрыш)
    # если 2,4,6 - бомба не взрывается (проигрыш)
    if bet_type == "boom":
        win = v in [1, 3, 5]
    else:  # safe
        win = v in [2, 4, 6]
...
# ОБРАБОТЧИК БОУЛИНГ
# =====================================================
def process_bowling_bet(call, bet_type, amount):
    user_id = call.from_user.id
    user_data = get_user(user_id)
    balance = user_data["balance"]
    ...
    bet_configs = {
        "strike": {"name": "Страйк", "coef": 6, "win_value": 6},
        "miss": {"name": "Промах", "coef": 6, "win_value": 1},
    }
    ...
    win = (v == config["win_value"])
    
    if win:
        win_amount = round(amount * config['coef'], 2)
        win_after_commission = round(win_amount * 0.94, 2)
        user_data["balance"] += win_after_commission
        update_user(user_id, balance=user_data["balance"])
        ...
        result_text = (
            f"{name}, ⏫ *Выигрывает* *{format_amount(win_after_commission)}* 💲\n"
            f"*Ставка* *{format_amount(amount)}* $ × *{config['coef']}*\n\n"
            f"➕ *Ставка:* *{format_amount(amount)}* 💲\n"
            f"💰 *Баланс:* *{format_amount(user_data['balance'])}* 💲"
        )
    else:
...
def process_darts_bet(call, bet_type, amount):
    ...
    bet_configs = {
        "center": {"name": "Прямо в центр", "coef": 6, "win_value": 6},
...
def process_cube_bet(call, bet_type, amount):
    ...
    bet_configs = {
        ...
        "odd": {"name": "Нечёт", "coef": 2, "win_func": lambda v: v in [1, 3, 5]},
        ...
        "ladder": {"name": "Лесенка", "coef": None, "win_func": lambda v: True},
    }
...
def handle_all_callbacks(call: CallbackQuery):
    try:
        ...
        if data.startswith('bomb_'):
            ...
            process_bomb_bet(call, bet_type, amount)
            return
        
        # ===== БОУЛИНГ НАЗАД =====
        if data.startswith('bowling_back_'):
            amount = float(data.split('_')[2])
            bot.answer_callback_query(call.id)
            class FakeMessage:
                def __init__(self, chat_id, from_user):
        ...
        # ===== БОУЛИНГ СТАВКИ =====
        if data.startswith('bowling_'):
            parts = data.split('_')
            bet_type = parts[1]
            amount = float(parts[2])
            process_bowling_bet(call, bet_type, amount)
            return
        ...
        if data.startswith('game_'):
            ...
            if game == "football":
                ...
                bowling_menu(call, amount)
                return
            elif game == "bomb":
                bomb_menu(call, amount)
                return
            elif game == "cube":
