...
def games_menu(message, amount=None, result_text=None, reply_to_msg_id=None):
    ...
    markup.row(*row2)
    markup.add(
        InlineKeyboardButton("💣 (х1.9)", callback_data=f"game_bomb_{amount}")
    )
...
def bomb_menu(call, amount):
    ...
    markup.add(
        InlineKeyboardButton("💣 Взорвётся (x1.9)", callback_data=f"bomb_boom_{amount}"),
        InlineKeyboardButton("💣 Не взорвётся (x1.9)", callback_data=f"bomb_safe_{amount}")
    )
